...
export default function Home() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const storedColorScheme = localStorage.getItem('colorScheme') || 'light';
      setColorScheme(storedColorScheme);
      document.documentElement.setAttribute('data-theme', storedColorScheme);
    }
  }, []);
  ...
  const handleContentUpload = (itemId, content) => {
    setCalendarData((prevData) =>
      prevData.map((item) => (item.id === itemId ? { ...item, uploadedContent: content } : item)),
    );
  };
  ...
  const filteredData = calendarData.filter((item) => {
    const dayMatch = filters.days.includes(item.day.toLowerCase());
    const platformMatch =
      filters.platforms.includes('all') || filters.platforms.includes(item.platform.toLowerCase());
    return dayMatch && platformMatch;
  });
